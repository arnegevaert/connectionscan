    ...
    async getPage() {
        ...
        await new ldfetch().get(this.currentUrl).then(response => {
            ...
            connectionIDs.forEach(c => {
                ...
                let tripId = connectionTriples.getTriples(null, buildingBlocks['gtfs:trip'])[0];
                if (typeof departureStop !== 'undefined' &&
                    typeof arrivalStop !== 'undefined' &&
                    typeof departureTime !== 'undefined' &&
                    typeof arrivalTime !== 'undefined') {
                    let depTime = new Date(departureTime.object);
                    let arrTime = new Date(arrivalTime.object);
                    connections.push({
                        dep: {time: depTime.getTime(), stop: departureStop.object},
                        arr: {time: arrTime.getTime(), stop: arrivalStop.object},
                        tripId: tripId.object
                    });
                }
            });
            this.connections = connections;
            this.connections.sort((a,b) => b.dep.time - a.dep.time);
        });
    }
