        ...
        if (this._discoveryPhase) {
            ...
            this.ldfetch.get(this.starturl).then(response => {
                ...
                for (let i = 0; i < searchUriTriples.length; i ++) {
                    ...
                    let mappings = metaTriples.filter(triple => {
                        return triple.subject === searchUri && triple.predicate === 'http://www.w3.org/ns/hydra/core#mapping';
                    });

                    let mappingsUri = mappings[0].object;

                    /*for (let j = 0; j < mappings.length; j++) {
                      let mapping = metaTriples.filter(triple => {
                ...
                //Option 2: rangeGate and rangeFragments (multidimensional interfaces -- http://w3id.org/multidimensional-interface/spec)
                //Not yet implemented
            });
            ...
            if (this.store.connections.length > 0) {
                ...
                this.ldfetch.get(this.store.nextPageIri).then(response => {
                    this.store = new ConnectionsStore(response.url, response.triples);
                    let connection = this.store.connections.shift();
