...
class CSA {
    ...
    async getInterstopDistance(dep, arr) {
        if (this.enableISDCaches) {
            ...
            if (!found) {
                duration = await this.interstopDistanceCalculator.getInterstopDistance(dep, arr);
                this.ISDCache.push({stop1: dep, stop2: arr, duration: duration});
    ...
    discover(connection) {
        ...
        if (connection.departureTime > this.currentDepartureTime) {
            ...
            [connection.departureTime, connection.arrivalStop].forEach(stop => {
    ...
    incorporateInProfile(connection, ISD, stop, minVector) {
        ...
        let redundant = true;
        for (let i = 0; i < this.maxLegs; i++) {
            redundant = redundant && minVector[i] >= ISDDepEarliestEntry.arrTimes[i];
        }

        if (!redundant) {
            // If the new departure time is equal, update the profile entry
            // Else, insert a new entry
            let depTime = connection.departureTime - ISD.duration; // Calculate c_dep_time - f_dur
            let entry = {
    ...
    async calculateProfile(target, maxLegs, depTimeBounds) {
        ...
        while (connection !== null && connection.departureTime >= depTimeBounds.lower) {
