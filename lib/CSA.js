...
class CSA {
    ...
    incorporateInProfile(connection, ISD, stop, minVector) {
        ...
        if (!redundant) {
            ...
            let departureTime = new Date(connection.departureTime - ISD.duration); // Calculate c_dep_time - f_dur
            let entry = {
    ...
    async calculateConnectionVector(connection) {
        ...
        let x = Infinity;
        if (dist !== Infinity) {
            x = new Date(connection.arrivalTime + dist);
        }
    ...
    async calculateProfile(target, maxLegs, departureTimeBounds) {
        departureTimeBounds = {
            lower: departureTimeBounds.lower,
            upper: departureTimeBounds.upper
        };
