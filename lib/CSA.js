...
class CSA {
    ...
    async getJourneys(source, target, depTime, maxLegs, recalculateProfile = false) {
        if (recalculateProfile || !this.checkProfileMetadata(target, depTime, maxLegs)) {
            await this.calculateProfile(target, maxLegs);
        }
        return await this.journeyExtractor.extractJourneys(
            util.filterInfinity(this.stops, this.profile), source, target, depTime);
    }
    ...
    checkProfileMetadata(target, depTime, maxLegs) {
        return this.profileMetadata.target === target &&
            this.profileMetadata.depTime < depTime &&
