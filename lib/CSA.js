...
class CSA {
    ...
    async getInterstopDistance(departureStop, arrivalStop) {
        if (this.enableISDCaches) {
            ...
            if (!found) {
                duration = await this.interstopDistanceCalculator.getInterstopDistance(departureStop, arrivalStop);
                this.ISDCache.push({stop1: departureStop, stop2: arrivalStop, duration: duration});
                this.ISDCalls.uncached++;
            }
            return duration;
        }
        return await this.interstopDistanceCalculator.getInterstopDistance(departureStop, arrivalStop);
    }
    ...
    incorporateInProfile(connection, ISD, stop, minVector) {
        ...
        if (!redundant) {
            ...
            let departureTime = connection.departureTime - ISD.duration; // Calculate c_dep_time - f_dur
            let entry = {
    ...
    async calculateConnectionVector(connection) {
        ...
        let x = connection.arrivalTime + await this.getInterstopDistance(connection.arrivalStop, this.target);
     ...
     */
    async calculateProfile(target, maxLegs, departureTimeBounds) {
        departureTimeBounds = {
            lower: departureTimeBounds.lower.getTime(),
            upper: departureTimeBounds.upper.getTime()
        };
        ...
        this.reset(target, maxLegs, departureTimeBounds);
        ...
        this.profileMetadata = {
            target: target,
            departureTimeBounds: departureTimeBounds,
            maxLegs: maxLegs
        };
    ...
    async getJourneys(source, target, departureTimeBounds, maxLegs, recalculateProfile = false) {
        if (recalculateProfile || !this.checkProfileMetadata(target, departureTimeBounds, maxLegs)) {
            await this.calculateProfile(target, maxLegs, departureTimeBounds);
        }
        return await this.journeyExtractor.extractJourneys(
            util.filterInfinity(this.stops, this.profile), source, target, departureTimeBounds.lower);
    }
    ...
    checkProfileMetadata(target, departureTimeBounds, maxLegs) {
        return this.profileMetadata.target === target &&
            this.profileMetadata.departureTimeBounds.lower === departureTimeBounds.lower &&
            this.profileMetadata.departureTimeBounds.upper === departureTimeBounds.upper &&
            this.profileMetadata.maxLegs === maxLegs;
    }

    reset(target, maxLegs, departureTimeBounds) {
        this.stops = [];
        this.trips = [];
        this.profile = {};
        this.profileMetadata = {};
        ...
        this.target = target;
        if (departureTimeBounds) {
            this.connectionProvider.reset(departureTimeBounds);
        }
    }
