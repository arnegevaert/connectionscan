...
// TODO     Should we just use lc standard directly or should we view data transformation
// TODO     as a responsibility of ConnectionProvider?
class CSA {
    /**
        ...
        while (connection !== null) {
            ...
            [connection.dep.stop, connection.arr.stop].forEach(stop => {
                if (this.stops.indexOf(stop) === -1) {
                    profile[stop] = this.initProfileEntry(maxLegs);
            ...
            if (!dominated) {
                ...
                let ISCs = await this.interstopDistanceCalculator.getInterstopConnectionsForStop(connection.dep.stop);
    ...
    initProfileEntry(maxLegs) {
        return [{depTime: Infinity, arrTimes: Array(maxLegs).fill(Infinity),
